...
(function () {
  ...
  function main() {
    ...
    var samples, normaInfos = [], normaItems = [], itemsForNorma, j;
    var styleValidation;
    ...
    try {
      ...
      for (i = 0; i < exportEntries.length; i++) {
        ...
        for (j = 0; j < itemsForNorma.length; j++) {
          normaItems.push(itemsForNorma[j]);
        }
        normaInfos.push({
          index: i,
          entry: entry,
          ...
          startItem: exportStartItem,
          epigrafeItem: startItem,
          limitItem: limitItem,
          endItem: endItem,
          samples: samples
        });
        found++;
        nextStartAfter = startItem;
      }

      progress.update(0, "validando estilos somente dentro das normas", "Preparando");
      styleValidation = collectStyleValidation(normaItems, null);

      prefs = doc.htmlExportPreferences;
      snap = snapshotHtmlPrefs(prefs);
      configureHtmlPrefs(doc);
      try {
        for (i = 0; i < normaInfos.length; i++) {
          entry = normaInfos[i].entry;
          targetFolder = folderForEntry(setup.folder, normaInfos[i].folderInfo);
          file = new File(targetFolder.fsName + "/" + padNumber(i + 1, 3) + "_" + safeFileName(entry.title) + ".html");
          range = makeTextRangeFromItems(normaInfos[i].startItem, normaInfos[i].endItem);
          if (!range) {
            skipped++;
      ...
      for (i = 0; i < normaInfos.length; i++) {
        entry = normaInfos[i].entry;
        nextEntry = normaInfos[i].nextEntry;
        startItem = normaInfos[i].startItem;
        limitItem = normaInfos[i].limitItem;
        endItem = normaInfos[i].endItem;
        samples = normaInfos[i].samples;

        report.push("[OK] " + padNumber(i + 1, 3) + " | " + entry.title);
        report.push("  Pagina inicial: " + pageLabel(startItem));
        report.push("  Pagina final: " + pageLabel(endItem));
        report.push("  Paragrafos considerados: " + samples.total);
        report.push("  Arquivo HTML: " + (normaInfos[i].file ? normaInfos[i].file.fsName : "[nao exportado]"));
        report.push("  Limite: " + (nextEntry ? nextEntry.type + " - " + nextEntry.title : "fim da story/proxima epigrafe"));
        if (normaInfos[i].epigrafeItem && normaInfos[i].epigrafeItem.start !== startItem.start) {
          report.push("  Inicio ajustado por texto introdutorio: " + startItem.text);
        }
        report.push("  Epigrafe localizada: " + (normaInfos[i].epigrafeItem ? normaInfos[i].epigrafeItem.text : startItem.text));
        if (limitItem) report.push("  Primeiro paragrafo fora da norma: " + limitItem.text);
        pushSampleLines(report, "Dois primeiros paragrafos", samples.first);
        pushSampleLines(report, "Dois ultimos paragrafos", samples.last);
        report.push("");
      }
