...
async function buscarResenha(urlInformada) {
  const url = validarUrlResenha(urlInformada)
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`Planalto respondeu HTTP ${res.status}.`)
    }
    return { url, html: texto }
  } catch (error) {
    if (error?.name === 'AbortError') {
      throw new Error('Tempo esgotado ao acessar a resenha do Planalto.')
    }
    if (erroAcessoPlanalto(error)) {
      throw new Error('Nao foi possivel acessar automaticamente a resenha do Planalto.')
    }
    throw error
  } finally {
...
function credenciaisGoogle() {
  ...
  const userSecrets = path.join(app.getPath('userData'), 'secrets')
  const oauth = lerJson(path.join(rootSecrets, 'normando-gmail-oauth.json')) ||
    lerJson(path.join(userSecrets, 'normando-gmail-oauth.json')) ||
    {}
  const token = lerJson(path.join(rootSecrets, 'normando-gmail-token.json')) ||
    lerJson(path.join(userSecrets, 'normando-gmail-token.json')) ||
    {}
  const config = lerJson(path.join(rootSecrets, 'normando-gmail-config.json')) ||
    lerJson(path.join(userSecrets, 'normando-gmail-config.json')) ||
    {}
...
async function obterAccessToken() {
  ...
  if (!res.ok) {
    throw new Error(data.error_description || data.error || `Falha ao renovar token Gmail: HTTP ${res.status}`)
  }
...
async function confirmarAlteradoraNaCamara({ url, alteradora } = {}) {
  ...
  if (!chaveAlteradora.numero) {
    throw new Error('Norma alteradora nao identificada.')
  }
  const controller = new AbortController()
  const timeout = setTimeout(() => controller.abort(), 20000)
  try {
...
function coletarTextoPayload(payload, partes = []) {
  ...
  if (payload.body?.data) {
    ...
    if (payload.mimeType === 'text/html') {
      partes.push(htmlParaTexto(texto))
    } else if (!payload.mimeType || payload.mimeType === 'text/plain') {
      partes.push(texto)
    }
  }
...
async function buscarMensagemGmail(accessToken, id) {
  ...
  const data = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(data.error?.message || `Falha ao ler mensagem Gmail: HTTP ${res.status}`)
...
async function buscarResenhasGmail(opcoes = {}) {
  ...
  const data = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(data.error?.message || `Falha ao buscar Gmail: HTTP ${res.status}`)
...
export function registerResenhaHandlers() {
  ipcMain.handle('resenha:buscar', (_, url) => buscarResenha(url))
  ipcMain.handle('resenha:gmail', (_, opcoes) => buscarResenhasGmail(opcoes))
  ipcMain.handle('resenha:confirmar-camara', (_, payload) => confirmarAlteradoraNaCamara(payload))
  ipcMain.handle('resenha:vide-normas', (_, url) => buscarVideNormasCamara(url))
}
